...
//        modelAndView.addObject("ListzTree",tree);
    ...
    public PageBean<ElasticMenu> queryForPage(@RequestParam(value = "start", defaultValue = "1") int start, @RequestParam(value = "length", defaultValue = "10") int pageSize, @RequestParam(value = "date", required = false) String date, @RequestParam(value = "search", required = false) String search) {
        PageInfo<ElasticMenu> pageInfo = menuService.listForPage((start / pageSize) + 1, pageSize);
        return new PageBean<ElasticMenu>(pageInfo);
    }
    @Autowired
    ...
    public ModelAndView edit(@PathVariable String id, ModelAndView modelAndView){
        ElasticMenu menu = menuService.selectByPrimaryKey(id);
    ...
    public @ResponseBody
    Result update(ElasticMenu menu,String oldCode){
        Result<Integer> result = new Result<>();
        ...
        ElasticMenu temp = menuService.selectCode(menu.getCode());
